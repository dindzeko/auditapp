...
def app():
    ...
    if uploaded_files:
        st.subheader("📸 Preview Foto yang Diupload")
        col1, col2, col3 = st.columns(3)  # Membuat kolom untuk preview gambar
        coordinates = []
        valid_data = []
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            ...
            try:
                ...
                st.warning(f"Foto {uploaded_file.name} tidak dapat ditampilkan.")

            # Proses ekstraksi koordinat GPS
        ...
        if coordinates:
            ...
            st.info("Tidak ada foto dengan data GPS yang berhasil diproses.")
    else:
        st.info("Silakan upload foto dengan data GPS.")

    st.markdown('</div>', unsafe_allow_html=True)

# Blok untuk memastikan fungsi app() hanya dijalankan ketika file dijalankan secara langsung
