...
def app():
    # =====================================================
    # CATATAN PENTING (perubahan untuk integrasi ke AuditApp)
    # -----------------------------------------------------
    # st.set_page_config() DIHAPUS dari sini.
    # Alasan: fungsi ini dipanggil sebagai SUB-HALAMAN oleh app.py
    # (router utama). st.set_page_config() hanya boleh dipanggil SEKALI
    # dan HARUS menjadi perintah Streamlit PERTAMA di seluruh aplikasi.
    # Kalau dibiarkan di sini, akan memicu StreamlitAPIException saat
    # halaman Batch Bulanan dibuka.
    #
    # Kalau ingin layout "wide" untuk SELURUH aplikasi, tambahkan baris:
    #     st.set_page_config(page_title="AuditApp", layout="wide")
    # sebagai BARIS PERTAMA di app.py (sebelum add_css / perintah lain).
    # =====================================================
    ...
    with st.sidebar:
        st.header("⚙️ Panel Batch Bulanan")
